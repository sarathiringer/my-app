...
class Upload extends Component {
    ...
    renderActions() {
        if (this.state.successfulUploaded) {
            return (
                <button
                    onClick={() => this.setState({ files: [], successfulUploaded: false })
                    }
                >
                    Clear
                </button>
    ...
    async uploadFiles() {
        ...
        const promises = [];
        this.state.files.forEach(file => { promises.push(this.sendRequest(file));
        });
        try {
    ...
    sendRequest(file) {
        return new Promise((resolve, reject) => {
            ...
            req.onreadystatechange = () => {console.log(req.response)};
